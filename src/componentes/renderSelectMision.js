...
export function renderSelectedMision(mision) {
    ...
    const $mainCard = new ElementoBuilder("div").clase("w-full bg-slate-950/70 border border-slate-800 rounded-xl p-6 shadow-2xl flex flex-col lg:flex-row gap-8 items-center lg:items-stretch");
    
    // Sección izquierda
    const $leftSection = new ElementoBuilder("div").clase("flex flex-col sm:flex-row gap-6 flex-1 items-center sm:items-start w-full");
    const $imageContainer = new ElementoBuilder("img").clase("w-32 h-36 rounded-xl object-cover border border-slate-700 shadow-md shrink-0").atributo("src", mision.imagen).atributo("alt", mision.nombre);
    ...
    const $title = new ElementoBuilder("h2").clase("text-2xl font-bold text-white uppercase tracking-wide").texto(mision.nombre);
    const $badge = new ElementoBuilder("span").clase(`text-[9px] font-bold px-2 py-0.5 rounded border tracking-wider ${dificultadColor}`).texto(`● DIFICULTAD ${mision.dificultad}`);
    const $description = new ElementoBuilder("p").clase("text-xs text-slate-400 mt-2 leading-relaxed max-w-xl font-secondary").texto(mision.descripcion);
    const $metaInfo = new ElementoBuilder("div").clase("text-[10px] text-slate-500 space-y-0.5 font-medium tracking-wider mt-1");
    const $idRow = new ElementoBuilder("div").texto(`ID: ${mision.id}`);
    const $timeRow = new ElementoBuilder("div").texto(`TIEMPO: ${mision.tiempo} min`);
    const $rewardRow = new ElementoBuilder("div").texto(`RECOMPENSA: ${mision.recompensa} pts`);
    // Divisor
    const $divider = new ElementoBuilder("div").clase("hidden lg:block w-px bg-slate-900");
    
    // Sección derecha
    const $rightSection = new ElementoBuilder("div").clase("w-full lg:w-80 flex flex-col justify-center gap-4 shrink-0");
    ...
    const $requirementsContainer = new ElementoBuilder("div").clase("flex flex-col gap-2");
    ...
    const $specialtiesContainer = new ElementoBuilder("div").clase("flex flex-wrap gap-2");
    ...
    mision.requerimientos.forEach(([key, value]) => {
        ...
        const $iconSpan = new ElementoBuilder("span").inyectar(getRequerimientoIcon(key));
        const $labelText = new ElementoBuilder("span").texto(key.toUpperCase());
        const $valueSpan = new ElementoBuilder("span").clase("font-bold text-white").texto(value);
